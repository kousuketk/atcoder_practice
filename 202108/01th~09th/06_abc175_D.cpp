...
int main() {
  ...
  for(int i = 0; i < N; i++) {
    int next_i = vec_p[i];
    ll loop_size = 0, loop_sum_value = 0, loop_max = vec_c[next_i]; // 自分も含む(自分の要素は最後)
    ...
    if(loop_size >= K) {
      ...
      ans = max(ans, loop_max); // ループしない時
      ans = max(ans, tmp_loop_sum_value + tmp_max);
      ans = max(ans, tmp_loop_sum_value-loop_sum_value + loop_max); // 一個前のループで止める時
    }
  }
  cout << ans << endl;
}
...
// →今回は10^9が境界値だったから、10^9, -10^9を試してデバックした
// 「一個前のループで止める時」を考える必要があった→(制限時間内でムリムリ....)