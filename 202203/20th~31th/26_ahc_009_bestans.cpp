...
string create_str(int ti, int tj, int cnt_kabe_per = 0) {
  int cost = dist[ti][tj];
  int nowi = ti;
  int nowj = tj;
  string res = "";
  for(int _ = cost; _ >= 0; _--) {
    ...
    cost--;
    // 壁があったら指定の回数当たる
    if(nowi == 0 || vi[nowi-1][nowj] == '1') res += string(cnt_kabe_per, 'U');
    else if(nowi == 19 || vi[nowi+1][nowj] == '1') res += string(cnt_kabe_per, 'D');
    else if(nowj == 0 || vi[nowi][nowj-1] == '1') res += string(cnt_kabe_per, 'L');
    else if(nowj == 19 || vi[nowi][nowj+1] == '1') res += string(cnt_kabe_per, 'R');
  }
  reverse(ALL(res));
  return res;
}
...
int main() {
  ...
  string ans = create_str(ti, tj, cnt_kabe_per);
  cout << ans << endl;
}